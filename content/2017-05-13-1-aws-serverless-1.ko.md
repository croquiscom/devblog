...
Serverless환경에서는 소스코드와 몇 번의 클릭만으로 서비스 위한 REST API를 만들 수 있고,  
수 많은 request에 대한 확장성에 대한 고민을 할 필요가 없으며,  
로그 및 다양한 모니터링툴들을 추가적인 설치 없이 사용할 수 있습니다.  
...
다양한 언어를 사용하여 코드를 작성할 수 있습니다. (현재는 Node.js, Java, C#, Python를 지원)
...
현재까진 SQS에서 Lamabda를 바로 실행시킬 수 없기 때문에(SQS는 메시지의 저장 및 전달만을 담당하므로),  
위와 같이 2개의 Lambda function이 필요하며, CloudWatch 등을 이용하여 Queue를 확인하는 Lambda function(Consumer)을 주기적으로 수행시켜줘야 합니다.  
...
[Create New Queue]버튼을 눌러 새로운 Queue를 생성합니다.
...
하단의 Dead-letter queue를 설정은 메시지가 성공적으로 처리 되지 못한 조건을 설정하고,  
성공적으로 처리되지 못한 메시지들을 따로 처리할 수 있도록 도와줍니다.  
...
[Queue Actions]에서 [sendMessage]를 선택합니다.  
...
QueueUrl은 이전에 생성한 SQS의 url을 넣어줍니다. AWS의 Console에서 확인할 수 있습니다.
```
...
위에서 경험해 보았듯이 Serveless 환경이 언어,설정 등의 자유도는 떨어지지만,   
서버 운영에 대한 노력을 많이 줄일 수 있는 장점이 있습니다.  
각자의 선호도나 요구사항에 맞춰서 사용하면 좋지 않을까 생각합니다만, 저는 Serveless환경을 더 선호합니다. ^^v

다음 글에서는 AWS의 API Gateway 및 Lambda, DynamoDB를 이용해서 REST API를 만들어보며  
Serveless에 더 Deep하게 사용해보도록 하고,  
