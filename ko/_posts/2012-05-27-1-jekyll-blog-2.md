...
다시 말해 경로가 달라지는 것 외에는 일반 페이지의 처리와 크게 다르지 않습니다.
다만 보통 사용자가 일반 페이지와 포스팅의 레이아웃을 달리 지정해서 둘을 구분합니다.
...
심지어 index.html에서 각 포스팅으로의 링크도 없는데, 이것들 Jekyll이 지원하는 것이 아닌
직접 구현해야 하는 기능입니다.
[Liquid]의 기능을 사용하면 모두 구현가능합니다만, 아무것도 없는 상태에서 이러한 기능을
모두 구현하기는 쉽지 않겠죠.
